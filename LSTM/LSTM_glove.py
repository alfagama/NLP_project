...
#----------------------------------
batch_size = 128
checkpoint1 = ModelCheckpoint("weights/LSTM_glove_best_model1.hdf5", monitor='val_accuracy', verbose=1, save_best_only=True, mode='auto', period=1,save_weights_only=False)
...
# Evaluate with Keras
#----------------------------------
loss, accuracy = model.evaluate(X_test, Y_test, verbose=0, batch_size=batch_size) #, precision, recall
...
folder = 'LSTM-glove-binary-trainableTrue-noPrecision'

# Save LSTM model
#----------------------------------
print("Saving LSTM model to disk")
model.save('models/'+ folder +'/LSTM_glove_model.h5')
#----------------------------------
...
with open('models/'+ folder +'/LSTM_glove_tokenizer.pickle', 'wb') as handle:
...
with open('models/'+ folder +'/LSTM_glove_model_JSON.pickle', "w") as json_file:
...
plt.savefig("plots/"+ folder +"/LSTM_glove_accuracy.png")
plt.show()
...
plt.xlabel('epoch')
plt.legend(['train', 'validation'], loc='upper left')
plt.savefig("plots/"+ folder +"/LSTM_glove_loss.png")
plt.show()
#----------------------------------
